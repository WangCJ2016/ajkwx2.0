...
export function initialAirCondition(){
  ...
  return function(dispatch){
    request.get(config.api.base + config.api.queryDeviceType,{token:token,deviceName:encodeURI('空调'),houseId:sessionStorage.getItem('houseId')})
    .then(res => {
      if(res&&res.success){
        deviceType = res.dataObject
        dispatch(initialData(deviceType,'deviceType'))
        request.get(config.api.base + config.api.queryHostDeviceByType,{token:token,houseId:sessionStorage.getItem('houseId'),deviceType:res.dataObject})
        .then(res => {
          let airs = []
          if(res&&res.success){
            if (deviceType==='VIRTUAL_AIR_REMOTE') {
              res.dataObject.map((air) => {
                let airInfo = {},coolWays,warmWays
                if(air.ways){
